...
class DualDobotController:
    ...
    def _pick_and_place_rs(
        ...
        def mv(x, y, z):
            if _abort_flag:
                raise RuntimeError("동작 중단 요청됨")
            dev.move_to(x, y, z, rs.home_r, wait=True)
    ...
    def _next_graveyard(self, attacker_color: chess.Color) -> tuple:
        if attacker_color == chess.WHITE:
            gx, gy = GRAVEYARD_B
            offset = self._gy_idx_b * 28
            self._gy_idx_b += 1
        else:
            gx, gy = GRAVEYARD_A
            offset = self._gy_idx_a * 28
            self._gy_idx_a += 1
        return (gx + offset, gy)
